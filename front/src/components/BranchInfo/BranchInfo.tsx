...
const BranchInfoView = compose(BranchInfoBorderBottom)(BranchInfoBase);

const SelectArrow = compose(SelectArrowPositionBaseline)(SelectArrowBase);

const SelectName = compose(SelectNameColorSecondary)(SelectNameBase);
...
const BranchInfo = ({ repositoryName, page }: IBranchInfoProps) => (
  <BranchInfoView border="bottom">
    {page !== Page.repositories && (
      <>
        <BranchInfoHeader>
          <BranchInfoName>{repositoryName}</BranchInfoName>
          <BranchInfoSelect className={cnSelect({ size: 'big' })}>
            <SelectName color="secondary">trunk</SelectName>
            <SelectArrow
              position="baseline"
              className={cnArrow({ state: 'down', color: 'secondary' })}
            />
          </BranchInfoSelect>
          <BranchInfoSearch />
        </BranchInfoHeader>
