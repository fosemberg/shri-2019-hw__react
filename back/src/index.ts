...
app.get(
  ...
  (
    {
      params: { repositoryId, commitHash },
      query: { pageSize, pageNumber },
    }: IParams<IWithRepositoryId & IWithCommitHash> &
      IQuery<IWithPageSize & IWithPageNumber>,
    res: Response
  ) =>
    execCommandWithRes(
...
app.get(
  '/api/repos/:repositoryId/tree/:commitHash',
  (
    {
      params: { repositoryId, commitHash },
    }: IParams<IWithRepositoryId & IWithCommitHash>,
...
app.get(
  ...
  (
    {
      params: { repositoryId, commitHash, 0: path },
    }: IParams<IWithRepositoryId & IWithCommitHash & IWithPath>,
    res: Response
  ) =>
    execCommandWithRes(
      `cd ${PATH_TO_REPOS}/${repositoryId} &&
...
app.get(
  '/api/repos/:repositoryId/blob/master/*',
  (
    {
      params: { repositoryId, 0: path },
    }: IParams<IWithRepositoryId & IWithPath>,
    res: Response
  ) => {
...
// /api/repos/cool-timer/blob/cool-branch/src/components/Header/index.tsx
app.get(
  '/api/repos/:repositoryId/blob/:commitHash/*',
  (
    {
      params: { repositoryId, commitHash, 0: path },
    }: IParams<IWithRepositoryId & IWithCommitHash & IWithPath>,
...
app.get(
  ...
  (
    ...
    return execCommandWithRes(
      ...
      res,
      (out: string) =>
        arrayFromOut(out).map((str: string) => {
          const arr = str.replace('	', ' ').split(' ');
          const name = arr[3].replace(path, '');
          const fileType =
            arr[1] === 'tree'
              ? 'dir'
              : name.toUpperCase().includes('README') ||
                name.slice(-3) === '.md'
              ? 'readme'
              : 'file';
          return {
